...
func SetupLoger(cfg *config.Config) (*slog.Logger, *os.File) {
	...
	log = log.With(slog.String("application", applicationName))
	// чтобы легче искать в общих логах
	log = log.With(slog.String("service_name", applicationName))
